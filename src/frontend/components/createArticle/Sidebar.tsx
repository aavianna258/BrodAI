...
const SidebarContainer = (props: {
    ...
    const { sidebarCollapsed, children } = props;
    return (
        <Box
            sx={{
...
export default function Sidebar(props: SidebarProps) {
    ...
    const renderAccordion = (
        ...
        <StyledAccordion
            expanded={
                !sidebarCollapsed && collapseActiveKeys.includes(panelKey)
            }
            onChange={() => {
                if (collapseActiveKeys.includes(panelKey)) {
                    setCollapseActiveKeys(
                        collapseActiveKeys.filter((k) => k !== panelKey)
                    );
                } else {
    ...
    return (
        <SidebarContainer sidebarCollapsed={sidebarCollapsed}>
            {/* The Panels */}
            <Box
                sx={{ padding: sidebarCollapsed ? "0.5rem" : "1rem", flex: 1 }}
            >
                {renderAccordion(
                    "refine",
                    "Refine Article",
